...
def EEA_Clamshell_1W_SC_After_Entering_Firemode(json_data, NCDComPort,
    ...
    EEA_Clamshell_1W_Short_Circuit_After_Entering_Firemode(serialControlObj, videoPath, itr, passed_executions, PowerPackComPort, NCDComPort)
    serialControlObj.disconnectSerialConnection()


# Purpose-To perform Normal Firing Scenarios
def EEA_Clamshell_1W_Short_Circuit_After_Entering_Firemode(serialControlObj, videoPath, itr, passed_executions, PowerPackComPort, NCDComPort):
    success_flag = True
    ...
    for i in range(total_firings):
        ...
        # Firing Test
        serialControlObj.clamshell_1w_SC_after_firing(clampingForce=clampingForce, firingForce=firingForce,
                                                      reload_parameters=reload_parameters)
    # removing adapter
    logs_to_compare = 'Remove EEA Adapter with Clamshell 1W Short Circuit'
