...
def EEARecoveryFailureIncrementCounts(r, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte,
    ...
    EEA_Recovery_Failure_Increment_Counts(serialControlObj, EEA_RELOAD_EEPROM_command_byte, NCDComPort,
...
def EEA_Recovery_Failure_Increment_Counts(serialControlObj, EEA_RELOAD_EEPROM_command_byte, NCDComPort,
                                          PowerPackComPort, BlackBoxComPort, USB6351ComPort, ArduinoUNOComPort,
                                          OUTPUT_PATH, videoPath, p):
    ...
    eepromProcedureCnt = serialControlObj.r['Procedure Count']
    eepromFireCnt = serialControlObj.r['Fire Count']

    print(TestBatteryLevelMax, TestBatteryLevelMin, clampingForce, firingForce, numberOfFiringsinProcedure,
          eepromProcedureCnt, eepromFireCnt)
    ...
    # Pre-conditioning:
    ##  STEP 1: Remove handle form charger
    ...
    ##  STEP 2: By Reset Clamshell - Clamshell Connecting
    ###################################  RESETTING CLAMSHELL   #############################################################################
    ...
    ##  STEP 3: Checking Battery RSOC
    ##  Desired RSOC Values  check
    ...
    ## STEP 1: Place and Remove Handle from the Charger
    ...
    ## STEP 3: TIP Protector Detection
    # my_Serthread.clearQue()
    # serialControlObj.wait(5)
    ...
    ## STEP 4: Record Power Pack and Adapter usage Counts
    ...
    AdapterEeProcedureCount, AdapterEeFireCount = GetAdapterEepromUsageCounts(PowerPackComPort)

    # Reading Adapter One-Wire usage Counts
    AdapterOwProcedureCount, AdapterOwFireCount = GetAdapterOnewireUsageCounts(PowerPackComPort)
    ...
    # serialControlObj.Recovery_items_Test_Results.append('Firing =' + str(i + 1))
    ...
    if serialControlObj.r['Reload Type'] == "EEA":
        ...
        # read_data1 = 200
        # serRangeSensor.flushOutput()
        # serRangeSensor.flush()            /// Used this serial port while detecting the TPD
        # serRangeSensor.flushInput()
        ...
        with nidaqmx.Task() as task:
            task.ao_channels.add_ao_voltage_chan('Dev1/ao0')
            print('1 Channel 1 Sample Write: ')
            print(task.write(2))  # Updated the ff to zero by Manoj Vadali
        my_Serthread.clearQue()
        serialControlObj.wait(5)
        ...
        # Turning OFF the Adapter One Wire
        ...
        serialControlObj.Recovery_items_Test_Results.append('Retracting:' + result)
        ...
        serialControlObj.Recovery_items_Test_Results.append('Unclamping:' + result)
        ...
        # STEP 13: Restoring Adapter Onewire
        # Turning OFF the Adapter One Wire
        serialControlObj.Switch_ONN_Relay(1, 2)  # B1:R2 - Turning Off the Adapter Onewire
        print(serialControlObj.Test_Results)
