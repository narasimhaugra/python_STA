...
def EEA_Adapter_UART_Rx_Tx_OC_After_Entering_Firemode(json_data, NCDComPort,
    ...
    EEA_Adapter_UART_Rx_Tx_Open_Circuit_After_Entering_Firemode(serialControlObj, videoPath, itr,
...
def EEA_Adapter_UART_Rx_Tx_Open_Circuit_After_Entering_Firemode(serialControlObj, videoPath, itr,
                                                                passed_executions, PowerPackComPort, NCDComPort):
    success_flag = True
    ...
    # serialControlObj.serPP.write(command_PING)
    # serialControlObj.serPP.write(command_ONEWIRE_GET_CONNECTED)
    # serialControlObj.serPP.write(command_ONEWIRE_READ_ADAPTER)
    #
    # for retry in range(5):
    #     read_data = serialControlObj.serPP.read(2)
    #     read_data = read_data + serialControlObj.serPP.read(read_data[1] - 2)
    #     read_data = [hex(x)[2:].zfill(2) for x in read_data]
    #     # print(" One wie read response ", readData)
    #     if (int(read_data[3], 16) == 73) and (int(read_data[4], 16) == 3):
    #         AdapterErrorRecoveryState = (((int(read_data[65], 16)) * 256) + (int(read_data[66], 16)))
    #
    #         if AdapterErrorRecoveryState :
    ...
    for i in range(total_firings):
        ...
        # Firing Test
        serialControlObj.adapter_UART_Rx_Tx_OC_after_firing(clampingForce=clampingForce, firingForce=firingForce,
