...
def NormalFiring(r, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte, NCDComPort,
    ...
    Normal_Firing(serialControlObj, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte, NCDComPort,
                 PowerPackComPort, BlackBoxComPort, USB6351ComPort, ArduinoUNOComPort, OUTPUT_PATH, videoPath, p)
    serialControlObj.disconnectSerialConnection()


# Purpose-To perform Normal Firing Scenarios
def Normal_Firing(serialControlObj, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte, NCDComPort,
                 PowerPackComPort, BlackBoxComPort, USB6351ComPort, ArduinoUNOComPort, OUTPUT_PATH, videoPath, p):
    ...
    #
    # Timestapms, Strings_from_PowerPack = ReadQue(30, my_Serthread.readQue)
    #serialControl.convertListtoLogFile(Strings_from_PowerPack, 'TotalLog.txt', fileOpenMode='a')
    # Strings_to_Compare = [' ****  ENTERING AO EGIA Rotate CW STATE  ****',
    #                       ' ****  ENTERING AO EGIA Idle STATE  ****']
    # result = Compare('Adapter CW Rotated', Strings_to_Compare, Strings_from_PowerPack)
...
#                command = bytes(serialControlObj.MULU_EEPROM_command_byte)
        ...
        rf = serialControlObj.ForceDecode(retractionForce)
        with nidaqmx.Task() as task:
            task.ao_channels.add_ao_voltage_chan('Dev1/ao0')
            print('1 Channel 1 Sample Write: ')
            print(task.write(rf))
        ...
        for item in serialControlObj.Normal_Firing_Test_Results:
            ...
            try:
                if (((str.split(item, ':', 1))[1]) == 'PASS'):
                    ...
                    temp ='FAIL'
                    print(str((serialControlObj.Normal_Firing_Test_Results[0] + ':  Failed')))
                    break
            except:
                pass
        if temp == 'PASS':
        ...
        serialControlObj.Test_Results.append(serialControlObj.Normal_Firing_Test_Results[0] + ':'+ temp)
