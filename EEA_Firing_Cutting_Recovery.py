...
def EEAFiringCuttingRecovery(json_data, NCDComPort,
    ...
    EEA_Firing_Cutting_Recovery(serialControlObj, videoPath, itr, passed_executions, PowerPackComPort, NCDComPort)
    serialControlObj.disconnectSerialConnection()


# Purpose-To perform Normal Firing Scenarios
def EEA_Firing_Cutting_Recovery(serialControlObj, videoPath, itr, passed_executions, PowerPackComPort, NCDComPort):
    success_flag = True
    ...
    # Cutting Recovery - Negative Case
    if "Cut Recovery Exit State" == scenario_name:
        ...
        serialControlObj.MCPThread.readingPowerPack.exitFlag = True
        serialControlObj.wait(2)

        # Reading Adapter Onewire Usage Counts
        AdapterOwFireCount, AdapterOwProcedureCount = serialControlObj.GetAdapterNvmUsageCounts()

        print(f"Adapter EEPROM Fire Count: {AdapterEEPROMFireCount}, Adapter EEPROM Procedure Count: {AdapterEEPROMProcedureCount}")
        print(f"Adapter Onewire Fire Count: {AdapterOwFireCount}, Adapter Onewire Procedure Count: {AdapterOwProcedureCount}")
    ...
    for i in range(total_firings):
        ...
        # Firing Test
        serialControlObj.cutting_recovery_test(clampingForce=clampingForce, firingForce=firingForce,
