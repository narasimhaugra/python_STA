...
Purpose: Adapter UART Rx Tx OC error in Firing"""
...
def Adapter_UART_Rx_Tx_OC_in_Fire_Mode(r, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte,
                                   NCDComPort,
    ...
    print('Adapter UART Rx Tx Open Circuit in Fire Mode')
    #r, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte,   CARTRIDGE_EEPROM_command_byte)
    ...
    Adapter_UART_Rx_Tx_Open_Circuit_in_Fire_Mode(serialControlObj, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte,
                                   NCDComPort,
                                   PowerPackComPort, BlackBoxComPort, USB6351ComPort, ArduinoUNOComPort, OUTPUT_PATH,
                                   videoPath, p)
    # time.sleep(5)
...
def Adapter_UART_Rx_Tx_Open_Circuit_in_Fire_Mode(serialControlObj, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte,
                                   NCDComPort,
                                   PowerPackComPort, BlackBoxComPort, USB6351ComPort, ArduinoUNOComPort, OUTPUT_PATH,
                                   videoPath, p):
    ...
    for i in range(serialControlObj.r['Num of Firings in Procedure']):
        ...
        if serialControlObj.r['Reload Type'] == "Tri-Staple":
            ...
            videoThread = OLEDRecordingThread.myThread((str(serialControlObj.r['Scenario Num']) + '#' + str(p+1) + serialControlObj.r['Test Scenario'] + '_' +
                                                        serialControlObj.r['Reload Type'] + '_' + str(i+1)), serialControlObj.videoPath)
            videoThread.start()
            my_Serthread.clearQue()
            serialControlObj.wait(5)
            serialControlObj.ConnectingLegacyReload()
        ...
        if serialControlObj.r['Reload Type'] == "MULU":
            videoThread = OLEDRecordingThread.myThread((str(serialControlObj.r['Scenario Num']) + '#' + str(p + 1) + serialControlObj.r[
        ...
        Timestamps, Strings_from_PowerPack = ReadingQue(my_Serthread.readQue)
        serialControl.convertListtoLogFile(Strings_from_PowerPack, str(videoPath + '\\TotalLog.txt'), fileOpenMode='a')
        Strings_to_Compare = locateStringsToCompareFromEvent('Adapter UART Rx Tx OC in Fire Mode')
        result = Compare('Adapter UART Rx Tx OC in Fire Mode', Strings_to_Compare, Strings_from_PowerPack)
        serialControlObj.Normal_Firing_Test_Results.append('Adapter UART Rx Tx OC in Fire Mode:'+ result)
        print("Step: Adapter UART Rx Tx OC in Fire Mode")
        print(serialControlObj.Normal_Firing_Test_Results)
        rlist = {'Test Step': ['Adapter UART Rx Tx OC in Fire Mode'], 'Test Result': [result]}
        df = df.append(pd.DataFrame(rlist))
        ...
        Timestamps, Strings_from_PowerPack = ReadingQue(my_Serthread.readQue)
        serialControl.convertListtoLogFile(Strings_from_PowerPack, str(videoPath + '\\TotalLog.txt'), fileOpenMode='a')
        Strings_to_Compare = locateStringsToCompareFromEvent('Firing with Adapter UART Rx Tx OC')
        result = Compare('Firing with Adapter UART Rx Tx OC', Strings_to_Compare, Strings_from_PowerPack)
        ...
        Strings_to_Compare = locateStringsToCompareFromEvent('Adapter UART Rx Tx OC Retracting')
        result = Compare('Adapter UART Rx Tx OC Retracting', Strings_to_Compare, Strings_from_PowerPack)
        ...
        Strings_to_Compare = locateStringsToCompareFromEvent('Unclamping with Adapter UART Rx Tx OC in Fire Mode')
        result = Compare('Unclamping with Adapter UART Rx Tx OC in Fire Mode', Strings_to_Compare, Strings_from_PowerPack)
