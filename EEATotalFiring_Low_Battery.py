...
def EEATotalFiring_Low_Battery (r, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte, NCDComPort,
    ...
    EEA_Total_Firing_Low_Battery(serialControlObj, EEA_RELOAD_EEPROM_command_byte, NCDComPort,
...
def EEA_Total_Firing_Low_Battery(serialControlObj, EEA_RELOAD_EEPROM_command_byte, NCDComPort,
                 PowerPackComPort, BlackBoxComPort, USB6351ComPort, ArduinoUNOComPort, OUTPUT_PATH, videoPath, p):
...
# MCPThread.readingPowerPack.exitFlag = True
    ...
    Strings_to_Compare = locateStringsToCompareFromEvent('Remove Low Battery Signia Power Handle from Charger')
    #print(Strings_from_PowerPack)

    result = Compare('Remove Low Battery Signia Power Handle from Charger', Strings_to_Compare, Strings_from_PowerPack)
    ...
    serialControlObj.Test_Results.append('Remove Low Battery Signia Power Handle from Charger:' + result)
    ...
    if serialControlObj.r['Reload Type'] == "EEA":
        ...
        serialControlObj.wait(2) # 27th nov
        serialControlObj.Switch_ONN_Relay(2, 5)  # B2:R5 - ON
        serialControlObj.wait(0.5)
