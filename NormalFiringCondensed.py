...
def NormalFiringCondensed(r, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte, NCDComPort,
    ...
    Normal_Firing_Condensed(serialControlObj, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte, NCDComPort,
                 PowerPackComPort, BlackBoxComPort, USB6351ComPort, ArduinoUNOComPort, OUTPUT_PATH, videoPath, p)
    serialControlObj.disconnectSerialConnection()


# Purpose-To perform Normal Firing Scenarios
def Normal_Firing_Condensed(serialControlObj, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte, NCDComPort,
                 PowerPackComPort, BlackBoxComPort, USB6351ComPort, ArduinoUNOComPort, OUTPUT_PATH, videoPath, p):
...
################################################################################################################
    ...
    #     #serialControlObj.wait(.2)
    #     serialControlObj.Switch_ONN_Relay(1, 4)
    #     serialControlObj.Switch_ONN_Relay(1, 2)
    #     serialControlObj.Switch_ONN_Relay(1, 3)
    #
    # print('ESD Test Complete')

   ########## ESD As Per Ivan/Ginto ###################
    for lo in range (5):

        serialControlObj.Switch_OFF_Relay(1, 4)
        serialControlObj.Switch_OFF_Relay(1, 3)
        serialControlObj.Switch_OFF_Relay(1, 2)
        serialControlObj.wait(.350)
        serialControlObj.Switch_ONN_Relay(1, 2)
        serialControlObj.wait(.050)

        serialControlObj.Switch_ONN_Relay(1, 4)
        serialControlObj.Switch_ONN_Relay(1, 3)
        print('ESD Test Complete')
        print('Connecting Legacy Reload')
        serialControlObj.ConnectingLegacyReload()
        serialControlObj.wait(100)
...
#                command = bytes(MULU_EEPROM_command_byte)
        ...
        with nidaqmx.Task() as task:
            task.ao_channels.add_ao_voltage_chan('Dev1/ao0')
            print('Setting Particle Brake Clamp Force:')
            print(task.write(cf))
        ...
        with nidaqmx.Task() as task:
            task.ao_channels.add_ao_voltage_chan('Dev1/ao0')
            print('Setting Particle Brake Firing Force ')
            print(task.write(ff))
        my_Serthread.clearQue()
        serialControlObj.wait(5)
        #serialControlObj.Switch_ONN_Relay(2, 5)  # B2:R5 - ON
        #serialControlObj.wait(30) # Commented and added below code Malla Venkata

        for z in range(0, 50):
            serialControlObj.Switch_ONN_Relay(2, 5)  # B2:R5 - ON
            serialControlObj.wait(0.50)
        ...
        with nidaqmx.Task() as task:
            task.ao_channels.add_ao_voltage_chan('Dev1/ao0')
            print('Setting Particle Brake Retracting Force ')
            print(task.write(rf))
