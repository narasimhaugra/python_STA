""" Written by Manoj Vadali, Date: 10-Jan-2022
Purpose: Adapter 1W Open circuit error during Firing"""
...
def Adapter_1W_OC_in_Fire_Mode(r, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte, NCDComPort,
    ...
    print('Adapter 1W Open Circuit in Fire Mode', r, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte)
   ...
   # below steps are for Adapter 1W Open Circuit in Firing Mode
    ...
    Adapter_1W_Open_Circuit_in_Fire_Mode(serialControlObj, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte, NCDComPort, PowerPackComPort, BlackBoxComPort, USB6351ComPort, ArduinoUNOComPort, OUTPUT_PATH, videoPath, p)
    # time.sleep(5)
    #
...
def Adapter_1W_Open_Circuit_in_Fire_Mode(serialControlObj, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte, NCDComPort, PowerPackComPort, BlackBoxComPort, USB6351ComPort, ArduinoUNOComPort, OUTPUT_PATH, videoPath, p):
    ...
    #     for p in range(0, numConnection):
    #         port = ports[p]
    #         strPort = str(port)
...
#    serialControlObj.wait(15)
    ...
    # = {'Test Step': ['Clamshell Connected'], 'Test Result': [result]} # df can be deleted
    #df = pd.DataFrame(#)
    ...
    # = {'Test Step': ['Adapter Connected'], 'Test Result': [result]}
    #(#))
    ...
    for i in range(serialControlObj.r['Num of Firings in Procedure']):
        ...
        if serialControlObj.r['Reload Type'] == "MULU":
            ...
            if i < 1:
                ...
                print(serialControlObj.Normal_Firing_Test_Results)
                # = {'Test Step': ['MULU Reload Connected'], 'Test Result': [result]}
                #(#))
            ...
            print(serialControlObj.Normal_Firing_Test_Results)
            # = {'Test Step': ['Cartridge Connected'], 'Test Result': [result]}
            #(#))
        ...
        # serialControlObj.wait(3)
        print(serialControlObj.Normal_Firing_Test_Results)
        # = {'Test Step': ['Clamp Cycle Test: Clamping'], 'Test Result': [result]}
        #(#))
        ...
        # = {'Test Step': ['Clamp Cycle Test: Un-Clamping'], 'Test Result': [result]}
        #(#))
        serialControlObj.wait(5)
        print(serialControlObj.Normal_Firing_Test_Results)
        ...
        # = {'Test Step': ['Clamping'], 'Test Result': [result]}
        #(#))
        ...
        print(serialControlObj.Normal_Firing_Test_Results)
        # = {'Test Step': ['Safety Key: Acknowledged'], 'Test Result': [result]}
        #(#))

        print("Disconnecting Adapter 1Wire")
        serialControlObj.Switch_OFF_Relay(1, 2)
        ...
        serialControlObj.Normal_Firing_Test_Results.append('Firing:')
        serialControlObj.Normal_Firing_Test_Results.append(result)
        # = {'Test Step': ['Firing'], 'Test Result': [result]}
        #(#))
        ...
        Strings_to_Compare = locateStringsToCompareFromEvent('Adapter 1W OC Retracting')
        result = Compare('Adapter 1W OC Retracting', Strings_to_Compare, Strings_from_PowerPack)
        serialControlObj.Normal_Firing_Test_Results.append('Retracting:' + result)
        print("Step: Retracting Done")
        print(serialControlObj.Normal_Firing_Test_Results)
        serialControlObj.wait(3)
        # = {'Test Step': ['Adapter 1W OC Retracting'], 'Test Result': [result]}
        #(#))
        ...
        # = {'Test Step': ['Un-Clamping'], 'Test Result': [result]}
        #(#))
        # TEST STEP: Remove Reload

        if serialControlObj.r['Reload Type'] == "SULU" or serialControlObj.r['Reload Type'] == "Tri-Staple":
            my_Serthread.clearQue()
            serialControlObj.wait(5)
            ...
            Strings_to_Compare = locateStringsToCompareFromEvent('Remove Reload - Adapter 1W OC in Fire Mode')
            result = Compare('Remove Reload - Adapter 1W OC in Fire Mode', Strings_to_Compare,
    ...
    if serialControlObj.r['Reload Type'] == "MULU":
        ...
        # Strings_to_Compare = [' GUI_NewState: EGIA_REQUEST_RELOAD']
        Strings_to_Compare = locateStringsToCompareFromEvent('Remove Reload - Adapter 1W OC in Fire Mode')
        result = Compare('Remove Reload - Adapter 1W OC in Fire Mode', Strings_to_Compare, Strings_from_PowerPack)
        serialControlObj.Test_Results.append('Remove MULU Reload:' + result)
        print("Step: Removed Reload")
        print(serialControlObj.Test_Results)
        serialControlObj.wait(2)

    # TEST STEP: Remove Adapter
    print("Connecting Adapter 1Wire")
    serialControlObj.Switch_ONN_Relay(1, 2)
    my_Serthread.clearQue()
    serialControlObj.wait(5)
