...
Purpose: Adapter UART Rx Tx Open Circuit  before Fire Mode"""
...
def Adapter_UART_Rx_Tx_OC_Before_Fire_Mode(r, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte,
                                 NCDComPort,
    ...
    print('Adapter UART Open Circuit Before Fire Mode', r, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte,
    ...
    Adapter_UART_Rx_Tx_Open_Circuit_Before_Fire_Mode(serialControlObj, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte,
...
def Adapter_UART_Rx_Tx_Open_Circuit_Before_Fire_Mode(serialControlObj, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte,
                                 NCDComPort, PowerPackComPort, BlackBoxComPort, USB6351ComPort, ArduinoUNOComPort, OUTPUT_PATH,
                                 videoPath, p):
    ...
    for i in range(serialControlObj.r['Num of Firings in Procedure']):
        ...
        Timestamps, Strings_from_PowerPack = ReadingQue(my_Serthread.readQue)
        serialControl.convertListtoLogFile(Strings_from_PowerPack, str(videoPath + '\\TotalLog.txt'), fileOpenMode='a')
        Strings_to_Compare = locateStringsToCompareFromEvent('Adapter UART Rx Tx OC Before Fire Mode')
        result = Compare('Adapter UART Rx Tx OC Before Fire Mode', Strings_to_Compare, Strings_from_PowerPack)
        serialControlObj.Test_Results.append('Adapter UART Rx Tx OC Before Fire Mode:'+ result)
        print("Step: Adapter UART Rx Tx OC Before Fire Mode")
        print(serialControlObj.Test_Results)
        rlist = {'Test Step': ['Adapter UART Rx Tx OC Before Fire Mode'], 'Test Result': [result]}
        df = df.append(pd.DataFrame(rlist))
#
        ...
        Strings_to_Compare = locateStringsToCompareFromEvent('Green Key ACK - Adapter UART Rx Tx OC Before Fire Mode')
        result = Compare('Green Key ACK - Adapter UART Rx Tx OC Before Fire Mode', Strings_to_Compare, Strings_from_PowerPack)
        serialControlObj.Test_Results.append('Green Key ACK:' + result)
        ...
        Strings_to_Compare = locateStringsToCompareFromEvent('Unclamping with Adapter UART Rx Tx OC Before Firing')
        result = Compare('Unclamping with Adapter UART Rx Tx OC Before Firing', Strings_to_Compare, Strings_from_PowerPack)
        serialControlObj.Test_Results.append('Unclamping with Adapter UART Rx Tx OC Before Firing:'+ result)
