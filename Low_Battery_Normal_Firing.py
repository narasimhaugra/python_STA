'''Created By Manoj Vadali on 3/March/2022 for Low Battery Normal Firing Scenario'''
...
def LowBatteryNormalFiring(r, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte, NCDComPort,
    ...
    print("-----------------Low Battery Normal Firing-------------------")
    ...
    Low_Battery_Normal_Firing(serialControlObj, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte, NCDComPort,
                 PowerPackComPort, BlackBoxComPort, USB6351ComPort, ArduinoUNOComPort, OUTPUT_PATH, videoPath, p)
    serialControlObj.disconnectSerialConnection()


# Purpose-To perform Normal Firing Scenarios
def Low_Battery_Normal_Firing(serialControlObj, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte, NCDComPort,
                 PowerPackComPort, BlackBoxComPort, USB6351ComPort, ArduinoUNOComPort, OUTPUT_PATH, videoPath, p):
    ...
    if (read_battery_RSOC(25, PowerPackComPort) > TestBatteryLevelMax): #and (read_battery_RSOC(25, PowerPackComPort) > TestBatteryLevelMin):
        ...
        while (read_battery_RSOC(50, PowerPackComPort)) > TestBatteryLevelMax:# and (read_battery_RSOC(25, PowerPackComPort) > TestBatteryLevelMin):
    ...
    Strings_to_Compare = locateStringsToCompareFromEvent('Remove Signia Power Handle from Charger Low Battery')
...
################################################################################################################
    ...
    Strings_to_Compare = locateStringsToCompareFromEvent('Adapter Connected Low Battery')
...
#                command = bytes(MULU_EEPROM_command_byte)
            ...
            if i < 1:
                ...
                '''Strings_to_Compare = [' ****  ENTERING AO EGIA Unclamp STATE  ****',
                                      ...
                                      ' Piezo: Ready Tone']'''
                Strings_to_Compare = locateStringsToCompareFromEvent('Clamp Cycle Test Un-Clamping Low Battery')
