""" Written by Manoj Vadali, Date: 19-Jan-2022
Purpose: Adapter 1W Short circuit error before Firing"""
...
def Adapter_1W_SC_Before_Fire_Mode(r, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte,
                                 NCDComPort,
    ...
    print('Adapter 1W Short Circuit Before Fire Mode') #, r, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte,  CARTRIDGE_EEPROM_command_byte)
    ...
    Adapter_1W_Short_Circuit_Before_Fire_Mode(serialControlObj, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte,
                                 NCDComPort, PowerPackComPort, BlackBoxComPort, USB6351ComPort, ArduinoUNOComPort, OUTPUT_PATH,
                                 videoPath, p)
    # time.sleep(5)
    #
...
def Adapter_1W_Short_Circuit_Before_Fire_Mode(serialControlObj, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte,
                                 NCDComPort, PowerPackComPort, BlackBoxComPort, USB6351ComPort, ArduinoUNOComPort, OUTPUT_PATH,
                                 videoPath, p):
    ...
    for i in range(serialControlObj.r['Num of Firings in Procedure']):
        ...
        serialControlObj.Switch_ONN_Relay(6, 6)
        serialControlObj.wait(3)
        if serialControlObj.r['Reload Type'] == "SULU" or serialControlObj.r['Reload Type'] == "MULU":
            Timestamps, Strings_from_PowerPack = ReadingQue(my_Serthread.readQue)
            serialControl.convertListtoLogFile(Strings_from_PowerPack, str(videoPath + '\\TotalLog.txt'), fileOpenMode='a')
            Strings_to_Compare = locateStringsToCompareFromEvent('Adapter 1W SC with Reload')
            result = Compare('Adapter 1W SC with Reload', Strings_to_Compare, Strings_from_PowerPack)
            serialControlObj.Normal_Firing_Test_Results.append('Adapter 1W SC with Reload:'+ result)
            print("Step: Adapter 1W Short Circuited")
            print(serialControlObj.Normal_Firing_Test_Results)
            rlist = {'Test Step': ['Adapter 1W SC with Reload'], 'Test Result': [result]}
            df = df.append(pd.DataFrame(rlist))
        ...
        serialControlObj.Normal_Firing_Test_Results.append('Firing:'+ result)
        ...
        Strings_to_Compare = locateStringsToCompareFromEvent('Adapter 1W SC Retracting')
        result = Compare('Adapter 1W SC Retracting', Strings_to_Compare, Strings_from_PowerPack)
        serialControlObj.Normal_Firing_Test_Results.append('Adapter 1W SC Retracting:' + result)
        print("Step: Retracting Done")
        print(serialControlObj.Normal_Firing_Test_Results)
        serialControlObj.wait(3)
        rlist = {'Test Step': ['Retracting'], 'Test Result': [result]}
        df = df.append(pd.DataFrame(rlist))
        ...
        if serialControlObj.r['Reload Type'] == "SULU" or serialControlObj.r['Reload Type'] == "Tri-Staple":
            ...
            Strings_to_Compare = locateStringsToCompareFromEvent('Remove Reload - Adapter 1W OC Before Fire Mode')
            result = Compare('Remove Reload - Adapter 1W OC Before Fire Mode', Strings_to_Compare,
...
#            serialControlObj.wait(5)
    ...
    if serialControlObj.r['Reload Type'] == "MULU":
        ...
        # Strings_to_Compare = [' GUI_NewState: EGIA_REQUEST_RELOAD']
        Strings_to_Compare = locateStringsToCompareFromEvent('Remove Reload - Adapter 1W OC Before Fire Mode')
        result = Compare('Remove Reload - Adapter 1W OC Before Fire Mode', Strings_to_Compare, Strings_from_PowerPack)
        serialControlObj.Test_Results.append('Remove MULU Reload:' + result)
        print("Step: Removed Reload")
        print(serialControlObj.Test_Results)
        serialControlObj.wait(2)
        print("Removing Adapter 1Wire Short Circuit")
        serialControlObj.Switch_OFF_Relay(6, 6)
        my_Serthread.clearQue()
        serialControlObj.wait(5)
