""" Name: Issue 417349 from Integrity - During clamping face toggle the reload one wire
Created by Ugra Date: 11-12-2024
Reference : EEA IS Protocol - EEA_IS_18_M - Reload Detection
Updates Case 1b: Reload Detection- MIN_ANVIL_DETECTION_FORCE Detected"""

from Serial_Relay_Control import serialControl


def EEAIsTotalFiring_417349(json_data, NCDComPort,
                            PowerPackComPort, BlackBoxComPort, USB6351ComPort, ArduinoUNOComPort, FtdiUartComPort,
                            OUTPUT_PATH, videoPath, itr, EEA_RELOAD_EEPROM_command_byte, passed_executions):
    ...
    EEA_IS_Total_Firing_417349(serialControlObj, videoPath, itr, passed_executions)
    serialControlObj.disconnectSerialConnection()


# Purpose-To perform Normal Firing Scenarios
def EEA_IS_Total_Firing_417349(serialControlObj, videoPath, itr, passed_executions):
    success_flag = True
    ...
    for i in range(total_firings):
        ...
        # Firing Test
        serialControlObj.firing_Issue_417349(reload_parameters=reload_parameters)
    # removing adapter
