'''Created on 23 Feb 2022 by Manoj Vadali. Function: Pressing random keys during retraction'''
'''import os
import sys
import serial
import datetime
...
def RandomKeyPressDuringRetraction(r, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte, NCDComPort,
    ...
    print("-----------------Random Key Press During Retraction-------------------")
    ...
    Random_Key_Press_During_Retraction(serialControlObj, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte, NCDComPort,
                 PowerPackComPort, BlackBoxComPort, USB6351ComPort, ArduinoUNOComPort, OUTPUT_PATH, videoPath, p)
    serialControlObj.disconnectSerialConnection()


# Purpose-To perform Normal Firing Scenarios
def Random_Key_Press_During_Retraction(serialControlObj, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte, NCDComPort,
                 PowerPackComPort, BlackBoxComPort, USB6351ComPort, ArduinoUNOComPort, OUTPUT_PATH, videoPath, p):
...
#                command = bytes(MULU_EEPROM_command_byte)
        ...
        #
        #
        # # TEST STEP: Entering Fire Mode - Tissue clamping
        #
        my_Serthread.clearQue()
        serialControlObj.wait(5)
        ...
        #serialControlObj.Switch_ONN_Relay()
        serialControlObj.Switch_ONN_Relay(2, 3)  # B2:R3 - ON articulation
        serialControlObj.wait(1)
        serialControlObj.Switch_OFF_Relay(2, 3)  # B2:R3 - ON articulation
        serialControlObj.Switch_ONN_Relay(3, 5)  # B3:R5 - ON # green key press& release
        serialControlObj.Switch_OFF_Relay(3, 5)  # B3:R5 - OFF
        serialControlObj.Switch_ONN_Relay(3, 5)  # B3:R5 - ON
        serialControlObj.wait(1)
        serialControlObj.Switch_OFF_Relay(3, 5)  # B3:R5 - OFF
        serialControlObj.Switch_ONN_Relay(2, 4)  # B2:R3 - ON OPEN KEY
        serialControlObj.wait(.5)
        serialControlObj.Switch_OFF_Relay(2, 4)  # B2:R3 - OFF OPEN KEY
        Timestamps, Strings_from_PowerPack = ReadingQue(my_Serthread.readQue)
        serialControl.convertListtoLogFile(Strings_from_PowerPack, str(videoPath + '\\TotalLog.txt'), fileOpenMode='a')
