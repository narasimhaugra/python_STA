...
def Adapter_1W_OC_Before_Fire_Mode(r, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte,
                               NCDComPort,
    ...
    print('Adapter 1W Open Circuit Before Fire Mode') #, r, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte,      CARTRIDGE_EEPROM_command_byte)
    ...
    Adapter_1W_Open_Circuit_Before_Fire_Mode(serialControlObj, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte,
                                     CARTRIDGE_EEPROM_command_byte, NCDComPort, PowerPackComPort, BlackBoxComPort,
...
def Adapter_1W_Open_Circuit_Before_Fire_Mode(serialControlObj, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte,
                                     CARTRIDGE_EEPROM_command_byte, NCDComPort, PowerPackComPort, BlackBoxComPort,
                                     USB6351ComPort, ArduinoUNOComPort, OUTPUT_PATH, videoPath,p):
   ...
   #    serialControlObj.wait(15)
    ...
    serialControlObj.wait(5)
    print('Clamshell Reset Byte array', command)
    s = ser.read(2)
    s = list(s)
    packet_size = s[1]
    ...
    for i in range(serialControlObj.r['Num of Firings in Procedure']):
        ...
        if serialControlObj.r['Reload Type'] == "SULU" or serialControlObj.r['Reload Type'] == "MULU":
            ...
            serialControl.convertListtoLogFile(Strings_from_PowerPack, str(videoPath + '\\TotalLog.txt'),
                                               fileOpenMode='a')
            Strings_to_Compare = locateStringsToCompareFromEvent('Adapter 1W OC Before Fire Mode')
            result = Compare('Adapter 1W OC Before Fire Mode', Strings_to_Compare, Strings_from_PowerPack)
            serialControlObj.Normal_Firing_Test_Results.append('Adapter 1W OC Before Fire Mode:' + result)
            print("Step: Disconnecting Adapter 1W Before Fire Mode")
            print(serialControlObj.Normal_Firing_Test_Results)
            rlist = {'Test Step': ['Disconnecting Adapter 1W Before Fire Mode'], 'Test Result': [result]}
            df = df.append(pd.DataFrame(rlist))
        ...
        # serialControlObj.Test_Results.append('Adapter 1W OC Retracting:')
        ...
        print(serialControlObj.Normal_Firing_Test_Results)
        serialControlObj.wait(3)
        rlist = {'Test Step': ['Adapter 1W OC Retracting'], 'Test Result': [result]}
        df = df.append(pd.DataFrame(rlist))
