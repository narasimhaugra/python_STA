""" Written by Manoj Vadali, Date: 18-Jan-2022
Purpose: Clamshell 1W Short circuit error before Firing"""
...
def Clamshell_1W_SC_Before_Fire_Mode(r, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte,
                                 NCDComPort,
    ...
    print('Clamshell 1W Short Circuit Before Fire Mode')#, r, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte,          CARTRIDGE_EEPROM_command_byte)
    ...
    Clamshell_1W_Short_Circuit_Before_Fire_Mode(serialControlObj, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte,
                                 NCDComPort,
                                 PowerPackComPort, BlackBoxComPort, USB6351ComPort, ArduinoUNOComPort, OUTPUT_PATH,
                                 videoPath, p)
    # time.sleep(5)
...
def Clamshell_1W_Short_Circuit_Before_Fire_Mode(serialControlObj, SULU_EEPROM_command_byte, MULU_EEPROM_command_byte, CARTRIDGE_EEPROM_command_byte,
                                 NCDComPort,
                                 PowerPackComPort, BlackBoxComPort, USB6351ComPort, ArduinoUNOComPort, OUTPUT_PATH,
                                 videoPath, p):
    ...
    for i in range(serialControlObj.r['Num of Firings in Procedure']):
        ...
        print("Short Circuiting Clamshell 1Wire")
        serialControlObj.Switch_ONN_Relay(6, 5)
        ...
        Strings_to_Compare = locateStringsToCompareFromEvent('Clamshell 1W SC Retracting')
        result = Compare('Clamshell 1W SC Retracting', Strings_to_Compare, Strings_from_PowerPack)
        serialControlObj.Normal_Firing_Test_Results.append('Clamshell 1W SC Retracting:' + result)
        ...
        if serialControlObj.r['Reload Type'] == "SULU" or serialControlObj.r['Reload Type'] == "Tri-Staple":
            ...
            Strings_to_Compare = locateStringsToCompareFromEvent('Remove Reload - Clamshell 1W SC')
            result = Compare('Remove Reload - Clamshell 1W SC', Strings_to_Compare, Strings_from_PowerPack)
            serialControlObj.Normal_Firing_Test_Results.append('Remove Reload - Clamshell 1W SC:' +result)
    ...
    Timestamps, Strings_from_PowerPack = ReadingQue(my_Serthread.readQue)
    serialControl.convertListtoLogFile(Strings_from_PowerPack, str(videoPath + '\\TotalLog.txt'), fileOpenMode='a')
    Strings_to_Compare = locateStringsToCompareFromEvent('Remove Adapter - Clamshell 1W SC')
    result = Compare('Remove Adapter - Clamshell 1W SC', Strings_to_Compare, Strings_from_PowerPack)
    serialControlObj.Test_Results.append('Remove Adapter - Clamshell 1W SC:' + result)
